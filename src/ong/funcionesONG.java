...
public class funcionesONG {	
	public boolean subirActividad(String name, String lugar, String descripcion, String init_date, String end_date, int horas, String ong,int plazas) throws Exception {
		...
		boolean res = false;
		bd.readDataBase();
		query = "INSERT INTO eef_primera_iteracion.actividades(Titulo, Tipo, Horas, Fecha_Inicio, Fecha_Fin, Descripcion, ONG, Lugar, plazasDisponibles) VALUES('"+name+"', 4, "+horas+", '"+init_date+"', '"+end_date+"', '"+descripcion+"', '"+ong+"', '"+lugar+"','"+plazas+"');";
		res = bd.insert(query);
